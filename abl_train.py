...
for alpha in alphas:
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, labels1, labels2, _) in enumerate(train_loader):
            ...
            with torch.autocast(device.type):
                ...
                for i in range(len(labels)):
                    ...
                    logging('Train-{}'.format(alpha), output_names[i], log_writer, loss, mae, mse, rmse, pcc, ccc, iter_idx)
            ...
            log_writer.add_scalar('TotalLoss/{}-{}'.format('Train', alpha), loss, iter_idx)
        ...
        losses = val_log(log_writer, alpha, scale_factor, y_pred_AR, y_true_AR, y_pred_ECG, y_true_ECG, epoch, uid=alpha, val=True)
        loss = beta * losses[0] + gamma * losses[1]
        log_writer.add_scalar('TotalLoss/{}-{}'.format('Validation', alpha), loss, epoch)
        ...
        if loss_hist[-1] == min(loss_hist):
            torch.save({
                ...
                os.path.join(savemodel, 'alpha_{}.pth.tar'.format(alpha))
            )
        log_writer.flush()
